...
echo "Update config"
echo "server.host = `/sbin/ip route|awk '/default/ { print $3 }'`" > mayocat-shop/shop/application/config.ini
echo "server.port = 10000" >> mayocat-shop/shop/application/config.ini
echo "server.name = ${args[$ELEMENTS - 1]}" >> mayocat-shop/shop/application/config.ini
